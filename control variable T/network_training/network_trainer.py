...
class NetworkTrainer(object):
    def __init__(self, deterministic=True, fp16=False):
        ...
        self.dataset_tr = self.dataset_val = None  # do not need to be used
        # , they just appear if you are using the suggested load_dataset_and_do_split
        ...
        self.train_loss_MA_eps = 5e-3  # MA条件设置
        self.max_num_epochs = 700
        self.num_batches_per_epoch = 250
        self.num_val_batches_per_epoch = 50
        self.also_val_in_tr_mode = False
        self.lr_threshold = 1e-6 # 如果lr仍然高于这个阈值，网络将不会自动终止训练。
        ...
        ################# 保存checkpoints的设置##################################
        self.save_every = 10
        self.save_latest_only = True  # 如果为false，它将不存储/overwrite _latest，而是单独的创建中间时间的checkpoint
        self.save_intermediate_checkpoints = True  # 是否保存 checkpoint_latest
        self.save_best_checkpoint = False#我们未保存，进一步保证测试集与训练与关
        self.save_final_checkpoint = True  # 是否保存final checkpoint

    @abstractmethod
    def initialize(self, training=True):
               """
        创建 self.output_folder
        调用 self.initialize_network and self.initialize_optimizer_and_scheduler
    ...
    def save_checkpoint(self, fname, save_optimizer=True):
        ...
        save_this = {
            ...
            'plot_stuff': (self.all_tr_losses, self.all_val_losses, self.all_val_losses_tr_mode,
                           self.all_val_eval_metrics,self.SAloss),
    ...
    def load_checkpoint_ram(self, checkpoint, train=True):
        ...
        self.all_tr_losses, self.all_val_losses, self.all_val_losses_tr_mode, self.all_val_eval_metrics,self.SAloss = checkpoint[
            'plot_stuff']
        ...
        if self.epoch != len(self.all_tr_losses):
            ...
            self.SAloss =self.SAloss[:self.epoch]
    ...
    def run_training(self):
        ...
        while self.epoch < self.max_num_epochs:
            ...
            self.all_tr_losses.append(np.mean(train_losses_epoch))  # l是一个list,包含全部迭代的loss,得出的是一个epoch的loss
            self.temperature = self.coolingschedule(self.epoch, self.max_num_epochs)
        ...
        self.epoch -= 1  # 防止加载model_final_checkpoint时出现问题.
    ...
    def manage_patience(self):
        ...
        if self.patience is not None:
            ...
            # check if the current epoch is the best one according to moving average. If so
            # then save 'best' model
            # Do not use this for validation. This is intended for test set prediction only.
            #self.print_to_log_file("current best_val_eval_criterion_MA is %.4f0" % self.best_val_eval_criterion_MA)
            #self.print_to_log_file("current val_eval_criterion_MA is %.4f" % self.val_eval_criterion_MA)
####下面这一小部分我们没有利用，不保存best model,可注释
...
#######结束
    ...
    def update_train_loss_MA(self):
        if self.train_loss_MA is None:
            ...
            self.a = self.train_loss_MA_alpha - (1-self.temperature)*0.5
            #print("此时的温度是：",self.temperature)
            self.train_loss_MA = self.a * self.train_loss_MA + (1 - self.a) * \
                                 self.all_tr_losses[-1]
        self.MAloss.append(self.train_loss_MA)
        self.print_to_log_file("Moving average loss: %.4f" % self.MAloss[-1])
